...
template <typename T> bool check_and_write_headers(Stream & strm, Headers & headers, T header_writer, Error & error) {
...
bool Server::read_content_core(
  ...
  if (!req.has_header("Content-Length") && !detail::is_chunked_transfer_encoding(req.headers)) {
      ...
      if (s != INVALID_SOCKET) {
          ...
          size_t to_peek =
              (payload_max_length_ > 0) ? (std::min)(payload_max_length_ + 1, static_cast<size_t>(4096)) : 1;
...
bool Server::check_if_not_modified(const Request & req, Response & res, const std::string & etag, time_t mtime) const {
    ...
    if (req.has_header("If-None-Match")) {
        if (!etag.empty()) {
            ...
            auto ret =
                detail::split_find(val.data(), val.data() + val.size(), ',', [&](const char * b, const char * e) {
...
bool Server::check_if_range(Request & req, const std::string & etag, time_t mtime) const {
...
void ClientImpl::prepare_default_headers(Request & r, bool for_stream, const std::string & ct) {
    (void) for_stream;
    for (const auto & header : default_headers_) {
...
ClientImpl::StreamHandle ClientImpl::open_stream(const std::string & method,
                                                 const std::string & path,
                                                 const Params &      params,
...
ssize_t ClientImpl::StreamHandle::read_with_decompression(char * buf, size_t len) {
...
ssize_t ChunkedDecoder::read_payload(char * buf, size_t len, size_t & out_chunk_offset, size_t & out_chunk_total) {
    if (finished) {
        return 0;
    }
...
bool ChunkedDecoder::parse_trailers_into(Headers & dest, const Headers & src_headers) {
    stream_line_reader lr(strm, line_buf, sizeof(line_buf));
    if (!lr.getline()) {
        return false;
    }
...
ClientImpl::StreamHandle Client::open_stream(const std::string & method,
                                             const std::string & path,
                                             const Params &      params,
                                             const Headers &     headers,
                                             const std::string & body,
